 ...
 */
public class NettyTLSWebServer extends NettyWebServer {
    ...
    public static WebServer build(Config config, ServerConfiguration configuration, Routing defaultRouting) {
/*        String unpairedRoutings =
      ...
      /*  if (!unpairedRoutings.isEmpty()) {
        ...
        val result = new NettyTLSWebServer(
            config,
            configuration == null
     ...
     *                      routing is used.
     */
    private NettyTLSWebServer(
        Config serverConfig,
        ServerConfiguration config,
        Routing routing,
        Map<String, Routing> namedRoutings) {
        super(config, routing, namedRoutings);
        ...
        val commonNeedClient = serverConfig.get("ssl.need-client-auth").asBoolean().orElseGet(() -> Boolean.FALSE);
        val commonWantClient = serverConfig.get("ssl.want-client-auth").asBoolean().orElseGet(() -> Boolean.FALSE);
        var commonClientAuth = ClientAuth.NONE;
        if (commonNeedClient && !commonWantClient) {
            commonClientAuth = ClientAuth.REQUIRE;
        } else if (commonWantClient) {
            commonClientAuth = ClientAuth.OPTIONAL;
        }
        ...
        for (Map.Entry<String, SocketConfiguration> entry : sockets) {
            ...
            if (soConfig.ssl() != null) {
                ...
                var clientAuth = ClientAuth.NONE;
                val hasSockets = serverConfig.get("sockets." + name + ".need-client-auth").hasValue() ||
                    serverConfig.get("sockets." + name + ".want-client-auth").hasValue();
                val clientNeedClient = serverConfig.get("ssl.need-client-auth").asBoolean().orElseGet(() -> Boolean.FALSE);
                val clientWantClient = serverConfig.get("ssl.want-client-auth").asBoolean().orElseGet(() -> Boolean.FALSE);

                if (!hasSockets) {
                    clientAuth = commonClientAuth;
                } else if (clientNeedClient && !clientWantClient) {
                    clientAuth = ClientAuth.REQUIRE;
                } else if (clientWantClient) {
